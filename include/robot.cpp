...
void Robot::driveStraight(float percent, float dist, float accPercent) {
  float leftPos = leftMotorA.position(degrees);
  float rightPos = rightMotorA.position(degrees);
  ...
  while (currDist < target) {
    ...
    leftDist = fabs(leftMotorA.position(degrees) - leftPos);
    rightDist = fabs(rightMotorA.position(degrees) - rightPos);
    sumErr = std::max(-100.0f, std::min(100.0f, sumErr + currDist));
    float driftError = isTether? (rightDist - leftDist) * 0.05 : 0;
    setLeftVelocity(dist > 0 ? forward : reverse, modPercent * pidPercent * (1 + driftError));
    setRightVelocity(dist > 0 ? forward : reverse, modPercent * pidPercent * (1 - driftError));
  }
  stopLeft();
  stopRight();
}
...
void Robot::deploy() {
  yeet.spin(reverse, 100, percentUnits::pct);
  int milliseconds = vex::timer::system();
  while(vex::timer::system() < milliseconds + 1500) {}
  yeet.stop();
  yeet.setBrake(brakeType::coast);
}

void Robot::startYeet() {
  yeet.spin(reverse, 100, percentUnits::pct);
}
